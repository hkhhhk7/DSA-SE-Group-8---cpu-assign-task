...
#### 5. 初始狀態
此返回值僅在初始時間不為 0 （也就是有指定開始時間）時觸發，會是 assignTask 返回訊息的第一個物件。\
此物件包含以下屬性：`back0`, `back1`, `CPU0`, `CPU1`
- `back0` / `back1`: Number, 表示目前對應的 CPU 需要處理時間的末尾
- `CPU0` / `CPU1`: 表示目前 CPU Queue 內有的任務
